...
public async Task<ChatResponse> AskAsync(string question)
{
    try
    {
        ...
        _logger.LogInformation("Retrieving from Knowledge Base {KbId} for question: {Question}",
        ...
        _logger.LogInformation("Retrieved {Count} results from Knowledge Base",
            retrieveResponse.RetrievalResults.Count);

        // Step 2 — Generate answer with Nova Lite
        var prompt = $"""
                You are a helpful healthcare assistant for Douglass Hanly Moir Pathology and Sonic Healthcare.
